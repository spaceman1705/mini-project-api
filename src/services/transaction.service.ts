...
type TransactionStatus = "WAITING_CONFIRMATION" | "ACCEPTED" | "REJECTED";

export async function getOrganizerTransactions(organizerId: string) {
...
export async function getTransactionById(transactionId: string, organizerId: string) {
  ...
  if (transaction.event.organizerId !== organizerId) {
    throw createCustomError(403, 'You can only view transactions for your own events');
  }
...
export async function acceptTransaction(
  ...
  if (transaction.event.organizerId !== organizerId) {
    throw createCustomError(403, "Forbidden: You are not the organizer of this event");
  }

  // 3. Validasi Status Transaksi (Hanya bisa menyetujui yang masih menunggu)
  if (transaction.status !== "WAITING_CONFIRMATION") {
    throw createCustomError(400, `Cannot accept transaction with status: ${transaction.status}`);
  }
...
export async function rejectTransaction(
  ...
  if (transaction.status !== 'WAITING_CONFIRMATION') {
    throw createCustomError(400, 'Only transactions with WAITING_CONFIRMATION status can be rejected');
  }
...
export async function checkoutService({
  ...
  if (ticket.availableQuota < quantity) {
    throw createCustomError(400, "Not enough ticket quota");
  }
